...
Phase 2 imports the shared transaction, graph, window, and temporal-path
foundation from `../phase-1`, then layers identity evidence on top.
...
From the `ghost-chains` directory:

```bash
docker build -f phase-2/Dockerfile -t ghost-chains-phase-2 .
docker run --rm -p 8080:8080 ghost-chains-phase-2
```
